...
function createSite(db, body, actor = 'dashboard') {
  ...
  if (db.sites.some((s) => s.code === body.code.trim())) {
  ...
  const site = {
    ...
    pestPathway: body.pestPathway || null, // country_free | pest_free_area | registered_site
    createdAt: now(),
  };
  db.sites.push(site);
  audit(db, 'site.create', { siteId: site.id, code: site.code }, actor);
  return site;
}

function createCropCycle(db, body, actor = 'dashboard') {
  requireFields(body, ['siteId', 'variety', 'plantDate']);
...
function createHarvest(db, body, actor = 'dashboard') {
  ...
  audit(db, 'harvest.create', { harvestId: harvest.id }, actor);
  return harvest;
}
...
function getTlcDetail(db, tlcId) {
  ...
  const harvests = allocs.map((a) => ({
    ...a,
    harvest: db.harvests.find((h) => h.id === a.harvestId),
  }));
