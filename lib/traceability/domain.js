...
function createHarvest(db, body, actor = 'dashboard') {
  ...
  // Optional: set kilos in the same step (no second “confirm” needed)
  const qty = body.quantityKg != null ? Number(body.quantityKg) : NaN;
  if (qty > 0) {
    return confirmHarvest(db, harvest.id, { quantityKg: qty, supervisor: harvest.supervisor }, actor);
  }
